...
binds_cpp_path = Pathname.new("src/engine/mruby_binding.cpp")
binds_h_path = Pathname.new("src/engine/mruby_binding.h")
binds_cpp = File.read(binds_cpp_path)
bind_header =  File.read(binds_cpp_path.to_s.gsub(".cpp",".h"))
headers = []
installs = []
binds_cpp.lines.each do |line|
  if(line.include? "mrb_value MRubyBinding::")
    bind = line.gsub("mrb_value MRubyBinding::","").gsub("(mrb_state *mrb,mrb_value self)","").gsub("}","").gsub("{","")
